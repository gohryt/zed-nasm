...
struct NasmExtension {
    cached_binary_path: Option<String>,
}

const GITHUB_REPO: &str = "bergercookie/asm-lsp";
...
impl NasmExtension {
    fn language_server_binary_path(
        ...
        let arch_str: &str = match arch {
            ...
            zed::Architecture::X86 => return Err("Unsupported arch x86".into()),
        };

        let platform_str: &str = match platform {
            zed::Os::Mac => "apple-darwin",
            zed::Os::Linux => "unknown-linux-gnu",
            zed::Os::Windows => return Err("Unsupported platform Windows".into()),
        };

        let file_name = format!("asm-lsp-{arch_str}-{platform_str}");
...
impl zed::Extension for NasmExtension {
    fn new() -> Self {
        Self {
...
zed::register_extension!(NasmExtension);
